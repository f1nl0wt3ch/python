...
class TTFontFile:
    ...
    def extractInfo(self):
        ...
        if not unicode_cmap_offset and not unicode_cmap_offset12:
            die(
                'Font (' + self.filename + ') does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 3, encoding 10, format 12, or platform 0, any encoding, format 4)')
    ...
    def makeSubset(self, file, subset):
        ...
        with open(file, 'rb') as self.fh:
            ...
            post = b("\x00\x03\x00\x00") + substr(opost, 4, 12) + b(
