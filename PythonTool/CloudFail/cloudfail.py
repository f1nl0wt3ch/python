...
def check_for_wildcard(target):
    ...
    try:
        ...
        while choice is not 'y' and choice is not 'n':
            choice = input(
                "A wildcard DNS entry was found. This will result in all subdomains returning an IP. Do you want to scan subdomains anyway? (y/n): ")
        if choice is 'y':
            return False
        else:
            return True
...
parser.add_argument("-s", "--subdomains", help="name of alternate subdomains list stored in the data directory",
                    type=str)
